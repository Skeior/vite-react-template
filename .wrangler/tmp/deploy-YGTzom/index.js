...
const app = new Hono2();
app.get("/api/", (c) => c.json({ name: "Talha Karasu" }));
async function getOrCreateDevice(db, deviceId) {
  ...
  if (result) {
    return {
      ...
      motionDetectedAt: result.last_motion_time,
      // AdminPanel bekliyor
...
async function upsertDevice(db, device) {
...
app.post("/data", async (c) => {
  try {
    ...
    if (contentType.includes("application/json")) {
      ...
      if (bytes[0] === 2 && bytes[bytes.length - 1] === 3) {
        const mod = bytes[2];
        const prop = bytes[3];
        const len = bytes[4];
    ...
    if (merged.rentalActive && merged.tripId && body.lat && body.lon) {
      try {
        const trip = await c.env.DB.prepare("SELECT realtime_route FROM trips WHERE trip_id = ?").bind(merged.tripId).first();
        if (trip) {
          let routePoints = [];
          if (trip.realtime_route) {
            try {
              routePoints = JSON.parse(trip.realtime_route);
            } catch (e) {
              routePoints = [];
            }
          }
...
app.post("/admin/state", async (c) => {
  try {
    ...
    const stateToUpdate = { ...device };
    if (body.rentalActive !== void 0) stateToUpdate.rentalActive = body.rentalActive;
    if (body.parkMode !== void 0) stateToUpdate.parkMode = body.parkMode;
    if (body.gpsSend !== void 0) stateToUpdate.gpsSend = body.gpsSend;
    if (body.statsSend !== void 0) stateToUpdate.statsSend = body.statsSend;
  ...
  try {
    ...
    const devices = (results.results || []).map((row) => ({
      ...
      value: {
        ...
        motionDetectedAt: row.last_motion_time,
        // AdminPanel için
...
app.get("/admin/trips", async (c) => {
  try {
    ...
    const limit = Math.min(1e3, Number(url.searchParams.get("limit")) || 100);
    ...
    const trips = (results.results || []).map((row) => ({
      tripId: row.trip_id,
      deviceId: row.device_id,
      lat: row.lat,
      lon: row.lon,
...
app.get("/admin/trips/:tripId", async (c) => {
  try {
    const tripId = c.req.param("tripId");
    if (!tripId) return c.text("Missing tripId", 400);
    const trip = await c.env.DB.prepare("SELECT * FROM trips WHERE trip_id = ?").bind(tripId).first();
    if (!trip) {
    ...
    return c.json({
      ...
      routePoints
      // For backward compatibility with ClientPage
    });
  } catch (err) {
    console.error(err);
...
app.post("/rental/end", async (c) => {
  try {
    ...
    if (device.parkMode && device.parkStartTime) {
      ...
      const parkDurationSec = Math.floor(parkDurationMs / 1e3);
    ...
    const kmCost = totalDistance * 1;
    const driveCost = driveDuration / 60 * 2;
    const parkCost = finalParkDuration / 60 * 1;
    const totalCost = kmCost + driveCost + parkCost;
    const now = (/* @__PURE__ */ new Date()).toISOString();
    let savedRoute = [];
    if (device.tripId) {
      const trip = await c.env.DB.prepare("SELECT realtime_route FROM trips WHERE trip_id = ?").bind(device.tripId).first();
      if (trip && trip.realtime_route) {
        try {
          savedRoute = JSON.parse(trip.realtime_route);
        } catch (e) {
...
app.get("/rental/status/:deviceId", async (c) => {
  try {
    const deviceId = c.req.param("deviceId");
    const device = await getOrCreateDevice(c.env.DB, deviceId);
    if (!device) {
...
app.post("/rental/control/:deviceId", async (c) => {
  try {
    const deviceId = c.req.param("deviceId");
    const body = await c.req.json();
    ...
    if (gpsSend !== void 0) updated.gpsSend = gpsSend === true || gpsSend === "true";
    if (statsSend !== void 0) updated.statsSend = statsSend === true || statsSend === "true";
...
app.post("/trip/reset/:deviceId", async (c) => {
  try {
    const deviceId = c.req.param("deviceId");
    const device = await getOrCreateDevice(c.env.DB, deviceId);
    if (device) {
...
app.post("/admin/clear-all", async (c) => {
  try {
